...
def train_model(learning_rate,steps,batch_size,training_examples,training_targets,
  ...
  for period in range (0, periods):
    ...
    print("  period {0} : {1:.02f}".format(period, training_root_mean_squared_error))
...
predict_input_test_fn = lambda: my_input_fn(test_examples, test_targets, num_epochs=1, shuffle=False)
  
test_predictions = linear_regressor.predict(input_fn=predict_input_test_fn)
test_predictions = np.array([item['predictions'][0] for item in test_predictions])

test_rmse = math.sqrt(metrics.mean_squared_error(test_predictions, test_targets))
print("Final RMSE on test data {0:.4f}", test_rmse)
